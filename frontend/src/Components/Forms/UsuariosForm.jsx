...
import Select from '../FormControl/Selects'

//* COMPONENTES
import { SaveButton, CancelButton, NavigatorButton } from "../Buttons/Index";
import TransferItems from "../ListItens/TransferItens";
...
const FormUpdate =  React.memo(({ handleSubmit, changeForm, handleChange, values }) => {
  ...
  return (
		<form noValidate onSubmit={handleSubmit} className={loading ? classes.dialogLoader : null}>
			{loading ? (
				<CircularProgress />
			) : (
		<>
...
const FormActivedDisabled = React.memo(({ handleSubmit, values }) => {
	...
	return (
		<form onSubmit={handleSubmit}>
			<DialogContent dividers>
				<h4>
					Quer realmente {type === "disabled" ? 'desabilitar' : 'habilitar'} o usuário, {values.nome} ?
				</h4>
			</DialogContent>
			<DialogActions>
				<SaveButton />
...
const FormClients = React.memo(({ handleSubmit, changeForm, handleClients, values }) => { 
	const classes = useStyles();
...
const FactorForm = (props) => {
	...
	React.useEffect(() => {
		if (type !== 'insert') {
			const newClients = usuario && usuario.clients ? usuario.clients.map(client => client.id) : [];
