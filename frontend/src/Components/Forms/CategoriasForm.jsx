...
const FormInsert = () => {
	 ...
	 * @param {number} ID
	 */
	const handleSubCategoria = React.useCallback((action, ID) => {
		...
		<form noValidate onSubmit={handleSubmit}>
			{loading ? (
				<div className={classes.dialogLoader}>
					...
					<DialogActions>
						<SaveButton disabled={subcategorias && subcategorias.length > 0 ? false : true}/>
					</DialogActions>
				</>
				)}
		</form>
...
const FormUpdate = () => {
	const classes = useStyles();
	const { categoria, errors, setErrors,  handleActions, apiLoading } = useCategorias();
	const { subcategorias } = useSubCategorias();
	const { type , loading, setLoading, setOpen } = useDialog();
	const { values, setValues , handleChange } = useForm();
	...
	return(
		<form onSubmit={handleSubmit}>
			<DialogContent dividers>
				<h4>
					Quer realmente deletar a categorias, {categoria.nome} ?
				</h4>
			</DialogContent>
			<DialogActions>
				<SaveButton />
