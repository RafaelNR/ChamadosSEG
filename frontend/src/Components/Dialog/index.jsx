...
const Dialog = (props) => {
  ...
  const FormActivedDisabled = props.options.Insert ? props.options.ActivedDisabled : '';
      ...
      case "disabled":
			case "actived":
				return (
					<React.Fragment>
						<DialogTitle  
						title={ type === "disabled" ? `Desabilitar ${props.options.title}` : `Habilitar ${props.options.title}` } />
						<FormActivedDisabled />
					</React.Fragment>
				);
