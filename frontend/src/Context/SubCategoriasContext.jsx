...
import {
	InsertSchema,
	UpdateSchema,
} from "../Schemas/SubCategorias.Schema";
	...
	const getSubCategoria = useCallback(
		async (ID) => {
			try {
				setApiLoading(true);
				setErrors({})
				return Api.getByID("subcategorias", parseInt(ID))
					.then((resp) => {
						const { success, data } = resp.data;
	...
	const Actions = {
		async insert(subcategoria) {
			try {
				...
				setSubCategorias((values) =>
					values.concat(resp.data.data)
				);
				handleSnackBar({
					type: "success",
		...
		async update(subcategoria) {
			try {
				...
				setSubCategorias(subs => subs.map(s => newData.id === s.id ? newData : s));
				handleSnackBar({
					type: "success",
