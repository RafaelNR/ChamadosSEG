        ...
        <Main>
          <Container maxWidth={false} className={classes.container}>
            <Switch>
              ...
              />
              <Route
                path="/atividades/edit/:ticket"
                component={AtividadesEdit}
              />
              <Route
                path="/atividades/view/:ticket"
...
const AdminRoute = (props) => {
  const { role_id } = props;
  return <>{role_id === 1 ? <Route {...props} /> : <Redirect to="/" />}</>;
};

const AnalistaRoute = (props) => {
  const { role_id } = props;
  return <>{role_id <= 2 ? <Route {...props} /> : <Redirect to="/" />}</>;
};
